...
class ClientSession:
        ...
        try:
            async for out_msg in self.stub.ClientStream(queue_as_aiter(self.out_queue)):
                ...
                if kind == "sync":
                    ...
                    update_kind = out_msg.update.WhichOneof("kind")
                    if update_kind == 'resource':
                        resource: labgrid_coordinator_pb2.Resource = out_msg.update.resource
                        ...
                        resource_path: labgrid_coordinator_pb2.Resource.Path = out_msg.update.del_resource
                        await self.on_resource_changed(
                            resource_path.exporter_name,
                            resource_path.group_name,
                            resource_path.resource_name,
                            {}
                        )
                    elif update_kind == 'place':
                        place = out_msg.update.place
                        await self.on_place_changed(place)
                    elif update_kind == 'del_place':
                        place_name = out_msg.update.del_place
    ...
    async def add_match(self):
        ...
        place = self.get_idle_place()
        if place.acquired:
            raise UserError(f"can not change acquired place {place.name}")  # FIXME: done in coordinator?
    ...
    async def del_match(self):
        ...
        for pattern in self.args.patterns:
            ...
            request = labgrid_coordinator_pb2.DeletePlaceMatchRequest(placename=place.name, pattern=pattern)  # FIXME: rename?
    ...
    async def add_named_match(self):
        ...
        name = self.args.name
        if not 2 <= pattern.count("/") <= 3:
            raise UserError(f"invalid pattern format '{pattern}' (use 'exporter/group/cls/name')")  # FIXME: done in coordinator?
        if place.hasmatch(pattern.split("/")):
            raise UserError(f"pattern '{pattern}' exists")  # FIXME: done in coordinator?
        if '*' in pattern:
            raise UserError(f"invalid pattern '{pattern}' ('*' not allowed for named matches)")  # FIXME: done in coordinator?
        if not name:
    ...
    async def release(self):
        ...
        user = self.getuser()
        if not place.acquired:
            raise UserError(f"place {place.name} is not acquired")  # FIXME: done in coordinator?
        _, user = place.acquired.split('/')
        if user != self.getuser():
            if not self.args.kick:
                raise UserError(f"place {place.name} is acquired by a different user ({place.acquired}), use --kick if you are sure")  # pylint: disable=line-too-long # FIXME: done in coordinator?
    ...
    async def create_reservation(self):
        ...
        request = labgrid_coordinator_pb2.CreateReservationRequest(spec=filters, priority=prio)

        try:
            res = await self.stub.CreateReservation(request)
            await self.sync_with_coordinator()
        except grpc.aio.AioRpcError as e:
            raise ServerError(e.details())
    ...
    async def _wait_reservation(self, token, verbose=True):
        while True:
            ...
            try:
                await self.stub.PollReservation(request)
                config = await self.sync_with_coordinator()  # FIXME: really sync here?
            except grpc.aio.AioRpcError as e:
                raise ServerError(e.details())
    ...
    async def print_reservations(self):
        ...
        request = labgrid_coordinator_pb2.GetReservationRequest(token=token)

        try:
            revervations = await self.stub.GetReservations(request)
            config = await self.sync_with_coordinator()  # FIXME: really sync here?
        except grpc.aio.AioRpcError as e:
            raise ServerError(e.details())
...
def start_session(url, extra):
    ...
    url = proxymanager.get_grpc_url(url, default_port=20408)

    session = ClientSession(url, loop, **extra)
